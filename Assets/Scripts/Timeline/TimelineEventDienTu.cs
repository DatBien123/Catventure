...
public class TimelineEventDienTu : MonoBehaviour
{
    #region [New]
        ...
        if (currentMapSelected != null)
        {
            if (currentMapSelected.MapData.Data.Name == "Hà Nội" || currentMapSelected.MapData.Data.Name == "Hạ Long")
            {
                Topic topicFound = currentMapSelected.MapData.Data.topicList.Find(
                    topic => topic.topicName == "Điền từ"
                );
                return topicFound;
            }
        }

        Debug.LogWarning("Không tìm thấy topic 'Điền từ' hoặc map không phải 'Hà Nội'");
        return new Topic();
    }
    ...
    public TimelineAsset GetCorrespondingCutScene()
    {
        ...
        if (currentMapSelected != null)
        {
            if (currentMapSelected.MapData.Data.Name == "Hà Nội" || currentMapSelected.MapData.Data.Name == "Hạ Long")
            {
                ...
                {
                    Debug.LogWarning("Topic 'Điền từ' không có cutscene!");
                }
            }
        }
