...
public class TimelineEventNoiTu : MonoBehaviour
{
    #region [New]
        ...
        if (currentMapSelected != null)
        {
            if (currentMapSelected.MapData.Data.Name == "Hà Nội" || currentMapSelected.MapData.Data.Name == "Hạ Long")
            {
                Topic topicFound = currentMapSelected.MapData.Data.topicList.Find(
                    topic => topic.topicName == "Nối từ"
                );
                return topicFound;
            }
        }

        Debug.LogWarning("Không tìm thấy topic 'Nối từ' hoặc map không phải 'Hà Nội'");
        return new Topic();
    }
    ...
    public TimelineAsset GetCorrespondingCutScene()
    {
        ...
        if (currentMapSelected != null)
        {
            if (currentMapSelected.MapData.Data.Name == "Hà Nội" || currentMapSelected.MapData.Data.Name == "Hạ Long")
            {
                ...
                {
                    Debug.LogWarning("Topic 'Nối từ' không có cutscene!");
                }
            }
        }
